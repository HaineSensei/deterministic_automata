...
#[test]
fn deterministic_automaton_creation_and_state_queries() -> Result<(), String> {
    ...
    assert_eq!(automaton.update_state(&'a')?, BasicStateSort::Reject);
    assert_eq!(automaton.current_state_sort()?, BasicStateSort::Reject);
    
    assert_eq!(automaton.update_state(&'b')?, BasicStateSort::Accept);
    assert_eq!(automaton.current_state_sort()?, BasicStateSort::Accept);
